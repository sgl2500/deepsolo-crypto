...
class PumpFade1mStrategy(Strategy):
    name = "pump_fade_1m"

    def __init__(self, config: PumpFade1mConfig, volume_gate: VolumeGate | None = None):
        self.config = config
        self.volume_gate = volume_gate

    def prepare_symbol(self, inst_id: str, bars_1h: list[Bar], bars_1m: list[Bar]) -> list[Signal]:
        del bars_1m
        signals: list[Signal] = []
    ...
    def order_for_signal(
        ...
        if signal.inst_id in context.open_positions:
            return None
        entry = find_1m_entry(bars_1m, signal.confirm_ts, self.config)
        if entry is None:
            return None
        return OrderIntent(
            strategy=self.name,
            ...
            reason="pump_fade_1m_entry",
            attrs={
                **signal.attrs,
                "delay_min": round(entry.delay_min, 1),
                "trigger": entry.trigger,
                "entry_bar": "1m",
                "exit_hold_minutes": self.config.exit_hold_minutes,
            },
        )
