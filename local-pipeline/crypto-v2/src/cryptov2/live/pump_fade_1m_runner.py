...
from cryptov2.strategies.pump_fade_1m.bot_compat import find_1m_entry
from cryptov2.strategies.pump_fade_1m.config import PumpFade1mConfig


@dataclass(frozen=True, slots=True)
class PumpFade1mLiveRunnerConfig:
    dry_run: bool = True
    max_workers: int = 10
    scan_1h_limit: int = 5
    entry_1m_limit: int = 70


class PumpFade1mLiveRunner:
    """Dry-run first live loop for the high-frequency 1m pump-fade strategy."""

    def __init__(
        self,
        market: PumpFadeMarketData,
        state: PumpFadeStateStore,
        strategy_config: PumpFade1mConfig,
        runner_config: PumpFade1mLiveRunnerConfig | None = None,
        ticker_snapshots: TickerSnapshotStore | None = None,
    ):
        self.market = market
        self.state = state
        self.strategy_config = strategy_config
        self.runner_config = runner_config or PumpFade1mLiveRunnerConfig()
    ...
    def scan_watchlist(self, now_ts: int) -> list[dict[str, Any]]:
        ...
        if self.ticker_snapshots is not None:
            self.ticker_snapshots.write_snapshot(now_ts, tickers, source="live_1m_scan")
    ...
    def check_entries(self, watchlist: list[dict[str, Any]], now: datetime, now_ts: int) -> int:
        positions = self.state.load_positions()
        active_ids = {inst_id for inst_id, pos in positions.items() if pos.get("status") == "active"}
        opened = 0
        for signal in watchlist:
            ...
            if elapsed_min > self.strategy_config.entry_search_window_min:
                continue
            bars_1m = self.market.get_bars(inst_id, "1m", self.runner_config.entry_1m_limit)
            entry = find_1m_entry(
                bars_1m,
                confirm_ts,
    ...
    def open_position(
        ...
        positions = self.state.load_positions()
        if sum(1 for pos in positions.values() if pos.get("status") == "active") >= self.strategy_config.max_positions:
            return False
        ...
        exit_ts = self._exit_ts(entry_ts)
        ...
        positions[inst_id] = {
            ...
            "margin_usd": self.strategy_config.capital_per_trade_usd,
    ...
    def _exit_ts(self, entry_ts: int) -> int:
        if self.strategy_config.exit_hold_minutes == 120:
            return (entry_ts // HOUR_MS + 2) * HOUR_MS
        return entry_ts + self.strategy_config.exit_hold_minutes * 60_000
